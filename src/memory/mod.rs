...
impl MemoryStore {
    ...
    fn list_facts(&self, limit: usize, include_forgotten: bool) -> Result<Vec<MemoryHit>> {
        ...
        let status_filter = if include_forgotten {
        ...
        let sql = format!(
            "SELECT id, content, source, status, created_at, strength,
                    COALESCE(importance, 3), NULL, COALESCE(source_episode_ids, '[]'),
                    visibility, owner_principal, owner_display_name, subjects
             FROM facts {status_filter}{access_filter} ORDER BY updated_at DESC LIMIT 5000"
        );
        let conn = self.data_conn()?;
        let mut stmt = conn.prepare(&sql)?;
        ...
        while let Some(row) = rows.next()? {
            ...
            hits.push(MemoryHit {
                ...
                content,
                score: strength.clamp(0.0, 1.0) as f32 * 5.0 + importance.clamp(1, 5) as f32,
                timestamp,
                source,
                retention: None,
                visibility,
